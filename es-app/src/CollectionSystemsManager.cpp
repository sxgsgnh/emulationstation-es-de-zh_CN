                ...
                if (curSys->isGroupedCustomCollection()) {
                    ...
                    mWindow->queueInfoPopup(
                        "禁用 '" +
                            ...
                            "' 在 '" + Utils::String::toUpper(sysData.system->getName()) + "'",
                        4000);
                }
                else {
                    ViewController::getInstance()->getGamelistView(curSys).get()->remove(
...
void CollectionSystemsManager::setEditMode(const std::string& collectionName, bool showPopup)
{
    ...
    if (showPopup) {
        mWindow->queueInfoPopup("编辑中 '" + Utils::String::toUpper(collectionName) +
                                    "' 专辑, 用'Y'添加/移除游戏 ",
                                10000);
    }
}
...
{
    if (showPopup) {
        mWindow->queueInfoPopup("完成编辑 '" + Utils::String::toUpper(mEditingCollection) +
                                    "' 专辑",
                                4000);
    }
...
const bool CollectionSystemsManager::toggleGameInCollection(FileData* file)
{
    if (file->getType() == GAME) {
        bool adding {true};
        std::string name {file->getShowName()};
        ...
        if (adding) {
            mWindow->queueInfoPopup(
                "添加 '" + Utils::String::toUpper(Utils::String::removeParenthesis(name)) +
                    "' 到 '" + Utils::String::toUpper(sysName) + "'",
                4000);
        }
        else {
            mWindow->queueInfoPopup(
                "移除 '" + Utils::String::toUpper(Utils::String::removeParenthesis(name)) +
                    "' 从 '" + Utils::String::toUpper(sysName) + "'",
                4000);
        }
        return true;
    }
...
void CollectionSystemsManager::deleteCustomCollection(const std::string& collectionName)
{
    ...
    if (collectionEntry != mCustomCollectionSystemsData.end()) {
        ...
        mWindow->queueInfoPopup(
            "删除专辑 '" + Utils::String::toUpper(collectionName) + "'", 5000);
    }
    else {
