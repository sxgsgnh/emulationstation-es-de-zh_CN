    ...
    , mRetryAccumulator {0}
{
    ...
    mResultName =
        std::make_shared<TextComponent>("结果名称", Font::get(FONT_SIZE_MEDIUM), 0x777777FF);
    ...
    mResultDesc =
        std::make_shared<TextComponent>("结果描述", Font::get(FONT_SIZE_SMALL), 0x777777FF);
    ...
    if (mScrapeRatings)
        mMD_Pairs.push_back(MetaDataPair(
            std::make_shared<TextComponent>("评级:", font, mdLblColor), mMD_Rating, false));

    mMD_Pairs.push_back(MetaDataPair(std::make_shared<TextComponent>("发布日期:", font, mdLblColor),
                                     mMD_ReleaseDate));
    mMD_Pairs.push_back(MetaDataPair(
        std::make_shared<TextComponent>("开发商:", font, mdLblColor), mMD_Developer));
    mMD_Pairs.push_back(MetaDataPair(
        std::make_shared<TextComponent>("发行商:", font, mdLblColor), mMD_Publisher));
    mMD_Pairs.push_back(
        MetaDataPair(std::make_shared<TextComponent>("游戏类型:", font, mdLblColor), mMD_Genre));
    mMD_Pairs.push_back(
        MetaDataPair(std::make_shared<TextComponent>("玩家数:", font, mdLblColor), mMD_Players));
...
void GuiScraperSearch::onSearchDone(std::vector<ScraperSearchResult>& results)
{
    ...
    if (results.empty()) {
        ...
        if (!isValidConfiguredScraper()) {
            ...
            row.addElement(std::make_shared<TextComponent>("没有游戏被找到", font, color), true);
