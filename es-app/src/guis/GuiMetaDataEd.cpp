    ...
    , mInvalidFolderLinkEntry {false}
{
    ...
    mTitle = std::make_shared<TextComponent>("编辑元数据", Font::get(FONT_SIZE_LARGE),
    ...
    buttons.push_back(std::make_shared<ButtonComponent>("保存", "保存元数据", [&] {
        save();
        delete this;
    }));
    buttons.push_back(
        std::make_shared<ButtonComponent>("取消", "取消修改", [&] { delete this; }));
        ...
        if (mClearGameFunc) {
            ...
            auto clearSelfBtnFunc = [this, clearSelf] {
                mWindow->pushGui(new GuiMsgBox(getHelpStyle(),
                                               "这将删除该文件夹的任何媒体文件和\n"
                                               "GAMELIST.XML条目,但文件夹本身\n"
                                               "或其中的任何内容都不会被删除,\n"
                                               "您确定吗?\n",
                                               "确定", clearSelf, "取消", nullptr));
            };
            buttons.push_back(
                std::make_shared<ButtonComponent>("清除", "清除文件夹", clearSelfBtnFunc));
        }
    }
    else {
        if (mClearGameFunc) {
            ...
            auto clearSelfBtnFunc = [this, clearSelf] {
                mWindow->pushGui(new GuiMsgBox(getHelpStyle(),
                                               "这将删除该游戏的任何媒体文件和\n"
                                               "GAMELIST.XML条目,但游戏文件本身不会被删除\n"
                                               "您确定吗\n",
                                               "确定", clearSelf, "取消", nullptr));
            };
            buttons.push_back(
                std::make_shared<ButtonComponent>("清除", "清除文件", clearSelfBtnFunc));
        }
        ...
        if (mDeleteGameFunc && !Utils::FileSystem::isDirectory(scraperParams.game->getPath())) {
            ...
            auto deleteGameBtnFunc = [this, deleteFilesAndSelf] {
                mWindow->pushGui(new GuiMsgBox(getHelpStyle(),
                                               ...
                                               "和GAMELIST.XML条目\n"
                                               "您确定吗\n",
                                               "确定", deleteFilesAndSelf, "取消", nullptr));
            };
            buttons.push_back(
                std::make_shared<ButtonComponent>("删除", "删除文件", deleteGameBtnFunc));
        }
    }
