    ...
    , mDeletedCustomCollection {false}
{
    ...
    for (std::map<std::string, CollectionSystemData, StringComparator>::const_iterator it =
        ...
        mCollectionSystemsAuto->add(it->second.decl.fullName, it->second.decl.name,
                                    it->second.isEnabled);
    addWithLabel("自动游戏专辑", mCollectionSystemsAuto);
    ...
    addWithLabel("自定义游戏专辑", mCollectionSystemsCustom);
    ...
    if (unusedFolders.size() > 0) {
        ...
        auto themeCollection = std::make_shared<TextComponent>(
            "从主题创建自定义专辑", Font::get(FONT_SIZE_MEDIUM), 0x777777FF);
        ...
        row.makeAcceptInputHandler([this, unusedFolders] {
            auto ss = new GuiSettings("选择主题文件夹");
    ...
    ComponentListRow row;
    auto newCollection = std::make_shared<TextComponent>("创建新的自定义专辑",
                                                         Font::get(FONT_SIZE_MEDIUM), 0x777777FF);
    ...
    if (Settings::getInstance()->getBool("VirtualKeyboard")) {
        row.makeAcceptInputHandler([this, createCollectionCall] {
            ...
            mWindow->pushGui(new GuiTextEditKeyboardPopup(
                getHelpStyle(), verticalPosition, "新专辑名字", "", createCollectionCall,
                false, "创建", "创建专辑?"));
        });
    }
    else {
        row.makeAcceptInputHandler([this, createCollectionCall] {
            mWindow->pushGui(new GuiTextEditPopup(getHelpStyle(), "新专辑名字", "",
                                                  createCollectionCall, false, "创建",
                                                  "创建专辑?"));
        });
    }
    addRow(row);
    ...
    auto deleteCollection = std::make_shared<TextComponent>(
        "删除自定义专辑", Font::get(FONT_SIZE_MEDIUM), 0x777777FF);
    ...
    auto collectionCustomGrouping = std::make_shared<OptionListComponent<std::string>>(
        getHelpStyle(), "专辑自定义组", false);
    const std::string& selectedCustomGrouping {
        Settings::getInstance()->getString("CollectionCustomGrouping")};
    collectionCustomGrouping->add("如果无主题", "unthemed", selectedCustomGrouping == "unthemed");
    collectionCustomGrouping->add("总是", "always", selectedCustomGrouping == "always");
    collectionCustomGrouping->add("从不", "never", selectedCustomGrouping == "never");
    ...
    addWithLabel("专辑自定义组", collectionCustomGrouping);
    ...
    addWithLabel("将收藏夹放在自定义收藏的顶部", fav_first_custom);
    ...
    addWithLabel("为自定义专辑展示星标", fav_star_custom);
