    ...
    , mApproveResults {approveResults}
{
    ...
    // Set up grid.
    mTitle = std::make_shared<TextComponent>("抓取处理中", Font::get(FONT_SIZE_LARGE),
                                             0x555555FF, ALIGN_CENTER);
    mGrid.setEntry(mTitle, glm::ivec2 {0, 0}, false, true, glm::ivec2 {2, 2});

    mSystem = std::make_shared<TextComponent>("系统", Font::get(FONT_SIZE_MEDIUM), 0x777777FF,
                                              ALIGN_CENTER);
    mGrid.setEntry(mSystem, glm::ivec2 {0, 2}, false, true, glm::ivec2 {2, 1});

    mSubtitle = std::make_shared<TextComponent>("子标题文本", Font::get(FONT_SIZE_SMALL),
    ...
    if (mApproveResults) {
        buttons.push_back(std::make_shared<ButtonComponent>("筛选搜索", "筛选搜索", [&] {
        ...
        buttons.push_back(std::make_shared<ButtonComponent>("跳过", "跳过游戏", [&] {
    ...
    buttons.push_back(std::make_shared<ButtonComponent>("停止", "停止",
...
void GuiScraperMulti::doNextSearch()
{
    ...
    // Update subtitle.
    ss.str("");
    ss << "游戏 " << (mCurrentGame + 1) << " OF " << mTotalGames << " - " << folderPath
       << scrapeName
...
void GuiScraperMulti::finish()
{
    ...
    if (mTotalSuccessful == 0) {
        ...
        ss << mTotalSuccessful << " 游戏" << ((mTotalSuccessful > 1) ? "S" : "")
           << " 抓取成功";
    ...
    mWindow->pushGui(new GuiMsgBox(getHelpStyle(), ss.str(), "确定", [&] {
        mIsProcessing = false;
        delete this;
    }));
}
