    ...
    , mCustomCollectionSystem {nullptr}
{
    ...
    if (!mFromPlaceholder) {
        ...
        mJumpToLetterList = std::make_shared<LetterList>(getHelpStyle(), "跳转到...", false);
        ...
        if (system->getName() != "recent")
            mMenu.addWithLabel("跳转到...", mJumpToLetterList);
        ...
        if (!mIsCustomCollectionGroup) {
            ...
            mListSort = std::make_shared<SortList>(getHelpStyle(), "排序游戏按", false);
            ...
            if (system->getName() != "recent")
                mMenu.addWithLabel("排序游戏按", mListSort);
        }
    }
    ...
    if (!mIsCustomCollectionGroup && system->getRootFolder()->getChildren().size() > 0) {
        if (system->getName() != "recent" && Settings::getInstance()->getBool("GamelistFilters")) {
            row.elements.clear();
            row.addElement(std::make_shared<TextComponent>("游戏列表过滤器",
    ...
    // Add a dummy entry when applicable as the menu looks quite ugly if it's just blank.
             ...
             !mIsCustomCollection) {
        row.elements.clear();
        row.addElement(std::make_shared<TextComponent>("该系统没有游戏",
                                                       Font::get(FONT_SIZE_MEDIUM), 0x777777FF),
                       true);
        mMenu.addRow(row);
    }
    ...
    if (UIModeController::getInstance()->isUIModeFull() &&
        ...
        if (CollectionSystemsManager::getInstance()->getEditingCollection() !=
            ...
            row.elements.clear();
            row.addElement(std::make_shared<TextComponent>("在该专辑上添加/移除游戏",
                                                           Font::get(FONT_SIZE_MEDIUM), 0x777777FF),
                           true);
    ...
    if (file->getType() == FOLDER) {
        if (UIModeController::getInstance()->isUIModeFull() && !mFromPlaceholder &&
            ...
            row.elements.clear();
            row.addElement(std::make_shared<TextComponent>("编辑该文件夹的元数据",
                                                           Font::get(FONT_SIZE_MEDIUM), 0x777777FF),
                           true);
            row.addElement(makeArrow(), false);
        ...
        if (UIModeController::getInstance()->isUIModeFull() && !mFromPlaceholder &&
            !(mSystem->isCollection() && file->getType() == FOLDER)) {
            row.elements.clear();
            row.addElement(std::make_shared<TextComponent>("编辑该游戏的元数据",
                                                           Font::get(FONT_SIZE_MEDIUM), 0x777777FF),
                           true);
            row.addElement(makeArrow(), false);
            row.makeAcceptInputHandler(std::bind(&GuiGamelistOptions::openMetaDataEd, this));
    ...
    if (file->getType() == FOLDER && file->metadata.get("folderlink") != "") {
        row.elements.clear();
        row.addElement(std::make_shared<TextComponent>("输入文件夹(覆盖文件夹链接)",
                                                       Font::get(FONT_SIZE_MEDIUM), 0x777777FF),
                       true);
    ...
    if ((!mIsCustomCollectionGroup && system->getRootFolder()->getChildren().size() == 0) ||
        ...
        mMenu.addButton("关闭", "关闭", [&] {
            mCancelled = true;
            delete this;
        });
    }
    else {
        mMenu.addButton("应用", "应用", [&] { delete this; });
        mMenu.addButton("取消", "取消", [&] {
            mCancelled = true;
            delete this;
        });
    }
...
std::vector<HelpPrompt> GuiGamelistOptions::getHelpPrompts()
{
    ...
    if (mSystem->getRootFolder()->getChildren().size() > 0 && mSystem->getName() != "recent") {
        prompts.push_back(HelpPrompt("b", "关闭(应用)"));
        prompts.push_back(HelpPrompt("back", "关闭(取消)"));
    }
    else {
        prompts.push_back(HelpPrompt("b", "关闭"));
        prompts.push_back(HelpPrompt("back", "关闭"));
    }
    return prompts;
}
