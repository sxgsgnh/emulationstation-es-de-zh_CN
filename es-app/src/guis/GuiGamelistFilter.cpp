...
void GuiGamelistFilter::initializeMenu()
{
    ...
    // Show filtered menu.
    row.elements.clear();
    row.addElement(std::make_shared<TextComponent>("重设所有过滤器", Font::get(FONT_SIZE_MEDIUM),
                                                   0x777777FF),
                   true);
    ...
    if (Settings::getInstance()->getBool("VirtualKeyboard")) {
        row.makeAcceptInputHandler([this, updateVal] {
            ...
            mWindow->pushGui(new GuiTextEditKeyboardPopup(
                getHelpStyle(), verticalPosition, "游戏名", mTextFilterField->getValue(),
                updateVal, false, "确定", "应用更改?"));
        });
    }
    else {
        row.makeAcceptInputHandler([this, updateVal] {
            mWindow->pushGui(new GuiTextEditPopup(getHelpStyle(), "游戏名",
