    ...
    if (input.value != 0) {
        if (config->isMappedTo("a", input)) {
            ...
            if (CollectionSystemsManager::getInstance()->isEditing() &&
                ...
                mWindow->queueInfoPopup("无法将自定义专辑添加到自定义专辑", 4000);
            }
            // Notify the user if attempting to add a placeholder to a custom collection.
            if (CollectionSystemsManager::getInstance()->isEditing() &&
                mRoot->getSystem()->isGameSystem() && getCursor()->getType() == PLACEHOLDER) {
                NavigationSounds::getInstance().playThemeNavigationSound(FAVORITESOUND);
                mWindow->queueInfoPopup("不能添加占位符到自定义专辑", 4000);
            }
                ...
                if (entryToUpdate->getType() == FOLDER) {
                    if (isEditing) {
                        mWindow->queueInfoPopup("不能添加文件夹到自定义专辑", 4000);
                    }
                    else {
                    ...
                    mWindow->queueInfoPopup("不能将无计数的条目添加到自定义专辑中",
                                            4000);
                }
    ...
    if (files.size() > 0) {
        for (auto it = files.cbegin(); it != files.cend(); ++it) {
            ...
            if (mCarousel != nullptr) {
                ...
                if (indicators == "none") {
                    name = inCollectionPrefix + (*it)->getShowName();
                }
                else {
