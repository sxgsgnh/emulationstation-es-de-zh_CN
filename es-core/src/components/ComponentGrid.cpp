...
std::vector<HelpPrompt> ComponentGrid::getHelpPrompts()
{
    ...
    if (!prompts.empty() && prompts.back() == HelpPrompt("up/down", "上/下选择")) {
        canScrollVert = true;
        if (canScrollHoriz && canScrollVert)
            prompts.pop_back();
    }
    else if (!prompts.empty() && prompts.back() == HelpPrompt("left/right", "左/右选择")) {
        canScrollHoriz = true;
        if (canScrollHoriz && canScrollVert)
            prompts.pop_back();
    }
    ...
    if (canScrollHoriz && canScrollVert)
        prompts.push_back(HelpPrompt("up/down/left/right", "上/下/左/右选择"));
    else if (canScrollHoriz)
        prompts.push_back(HelpPrompt("left/right", "左/右选择"));
    else if (canScrollVert)
        prompts.push_back(HelpPrompt("up/down", "上/下选择"));
