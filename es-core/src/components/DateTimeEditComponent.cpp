...
std::vector<HelpPrompt> DateTimeEditComponent::getHelpPrompts()
{
    ...
    if (!mEditing) {
        prompts.push_back(HelpPrompt("a", "编辑日期"));
    }
    else {
        prompts.push_back(HelpPrompt("b", "取消"));
        prompts.push_back(HelpPrompt("a", "应用"));
        prompts.push_back(HelpPrompt("left/right", "年-月-日"));
        prompts.push_back(HelpPrompt("up/down", "修改"));
    }
    return prompts;
}
