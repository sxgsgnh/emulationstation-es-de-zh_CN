...
bool Window::init()
{
    ...
    mSplashTextScanning = std::unique_ptr<TextCache>(mDefaultFonts.at(1)->buildTextCache(
        "搜索游戏中...", 0.0f, 0.0f, DEFAULT_TEXTCOLOR));
    mSplashTextPopulating = std::unique_ptr<TextCache>(
        mDefaultFonts.at(1)->buildTextCache("加载系统中...", 0.0f, 0.0f, DEFAULT_TEXTCOLOR));
    mSplashTextReloading = std::unique_ptr<TextCache>(
        mDefaultFonts.at(1)->buildTextCache("重新加载中...", 0.0f, 0.0f, DEFAULT_TEXTCOLOR));
